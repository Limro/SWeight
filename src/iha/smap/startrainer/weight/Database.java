package iha.smap.startrainer.weight;
...
public class Database
{
	...
	private static final String DATABASE_TABLE_ENTRY = "WeightTable";
	...
	{
        Log.i(TAG, "Removing database row, ID: " + rowIndex);
        open();
		boolean success = db.delete(DATABASE_TABLE_ENTRY, ENTRY_COL_ID + " = ?", new String[]{String.valueOf(rowIndex)}) > 0;
        close();
        return success;
	}
	...
	public Entry getEntry(long rowIndex)
	{
		...
		if (entry != null)
		{
			...
			{
				Entry e = new Entry(entry.getInt(COL_ID), //ID
                        entry.getDouble(COL_WEIGHT), //Weight
						entry.getString(COL_DATE)); //Date
				close();
				return e;
			}
			else
	...
	public List<Entry> getEntriesInPeriod(String from, String to)
	{
		...
		if (entry != null)
		{
			...
			if (entry.moveToFirst())
			{
				...
				{
					Entry next = new Entry(entry.getInt(COL_ID), //ID
                            entry.getDouble(COL_WEIGHT), //Weight
                            entry.getString(COL_DATE)); //Date
					list.add(next);
				}
	...
	public boolean updateEntry(long rowIndex, Entry myObject)
	{
        ...
        Log.i(TAG, "Update entry " + myObject.ID + " where " + where);
